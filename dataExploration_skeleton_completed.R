...
# install.packages("tidyverse", "corrplot", "stats")
...
prepped_df_base_male_chd <- filter(prepped_df_base, prepped_df_base$TenYearCHD == 1 & prepped_df_base$sex == "M")
...
my_vec <- c("High School Graduate", "College Graduate", "Some College")

male_grads <- prepped_df_tidy %>% 
  filter(education_class %in% my_vec & sex == "M")
