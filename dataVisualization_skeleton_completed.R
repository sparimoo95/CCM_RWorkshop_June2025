...
ggplot(prepped_df, aes(x = sex, y = BMI, fill = sex)) +
  geom_boxplot(alpha = .7, width = 0.36, notch = F, # `notch = TRUE` can be used to compare the median between groups based on overlap
...
ggplot(prepped_df, aes(x = age, y = totChol, color = sex)) +
